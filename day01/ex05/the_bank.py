...
def is_valid(account):
    if len(dir(account)) % 2 == 0:  # an even number of attributes,
        return False
    if any(attr.startswith('b') for attr in dir(account)):
        return False  # an attribute starting with b
    if not any(attr.startswith('zip') or attr.startswith('addr') for attr in
               dir(account)):  # no attribute starting with zip or addr
        return False
    if not hasattr(account, 'value') or not hasattr(account, 'name')\
            or not hasattr(account, 'id'):  # no attribute name, id and value
        return False
    if not isinstance(account.name, str):  # • name not being a string,
        return False
    if not isinstance(account.id, int):  # • id not being an int,
        return False
    if not isinstance(account.value, float)\
            and not isinstance(account.value, int):
        return False  # • value not being an int or a float.
    return True
...
class Bank(object):
    ...
    def transfer(self, origin, dest, amount):
        ...
        if amount < 0:
            return False
        origin_account = next((account for account in self.accounts
                               if account.name == origin), None)
        dest_account = next((account for account in self.accounts
    ...
    def __try_fix_account(self, account):
        ...
        if len(dir(account)) % 2 == 0:  # fix even number of attributes,
            while len(dir(account)) % 2 == 0:
                setattr(account, ''.join(random.choices(string.ascii_uppercase,
                                                        k=4)), 0)
        ...
        if not any(attr.startswith('zip') or attr.startswith('addr')
                   for attr in dir(account)):  # attribute starting zip or addr
            setattr(account, 'zip' +
                    ''.join(random.choices(string.ascii_uppercase, k=4)), 0)
        ...
        if not isinstance(account.id, int):  # id not being an int
            ...
            account.id = _id
        if not isinstance(account.value, float)\
                and not isinstance(account.value, int):  # not int or a float.
    ...
    def fix_account(self, name):
        ...
        if not isinstance(name, str):
            return False
        account = next(
            (account for account in self.accounts if account.name == name),
            None)
        if account is None:
            return False
        if is_valid(account):
