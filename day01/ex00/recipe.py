class Recipe:
    def __init__(self, name: str, cooking_lvl: int, cooking_time: int,
        ...
        if not isinstance(name, str) or len(name) == 0:
            ...
            exit(1)
        if not isinstance(cooking_lvl, int) or cooking_lvl < 1\
                or cooking_lvl > 5:
        ...
        if not isinstance(ingredients, list) or len(ingredients) == 0 or\
                not all([isinstance(elem, str) and len(elem) != 0
...
if __name__ == '__main__':
    Reciption = Recipe(
        'cake', 2, 30, ['hello', 'world'], 'description', 'dessert')
    to_print = str(Reciption)
    print(to_print)
